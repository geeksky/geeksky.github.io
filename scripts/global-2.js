...
function navSlide(){
	...
	for(i=0; i<sections.length; i++) {
		height.push(sections[i].offsetHeight);
	}
	...
	for(i=0; i<oBtn.length; i++) {
		oBtn[i].index = sections[i].index = i;
		oBtn[i].onclick = function(){
			var iTarget = pos[this.index];
			clearInterval(timer);
...
function workflowSlide() {
	...
	function resetList(current, next) {
		...
		oList1.style.left = "0";
		if(next > current) {			
			oList2.style.left = target + "px";//100%换成绝对数值 防止抖动
		}
		else if(next < current){
			oList2.style.left = -target + "px";
		}
